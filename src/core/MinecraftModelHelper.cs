...
public static class MinecraftModelHelper
{
    ...
    public static BlockState GetBlockStateByName(string name)
    {
        ...
        return prefixMatches.Count > 0 ? loader.GetBlockState(prefixMatches[0]) : null;
    }
    ...
    public static MeshInstance3D CreateMeshFromElement(ModelElement element, Dictionary<string, string> textures,
        Vector3 pivotOffset, Basis rotationBasis = default, bool hasRotation = false)
    {
    ...
    /// </summary>
    private static ArrayMesh CreateCubeMeshWithTextures(Vector3 from, Vector3 to, Dictionary<string, ElementFace> faces,
        Dictionary<string, string> textures, Basis rotationBasis = default, bool hasRotation = false)
    {
        ...
        // Down face (y-) - Bottom
        if (faces != null && faces.TryGetValue("down", out var face1))
        {
            var faceUVs = GetFaceUVs(face1);
        ...
        // Up face (y+) - Top
        if (faces != null && faces.TryGetValue("up", out var face2))
        {
            var faceUVs = GetFaceUVs(face2);
            AddFaceSurface(arrayMesh,
                new Vector3(-halfSize.X, halfSize.Y, -halfSize.Z),
                new Vector3(-halfSize.X, halfSize.Y, halfSize.Z),
                new Vector3(halfSize.X, halfSize.Y, halfSize.Z),
        ...
        // North face (z-)
        if (faces != null && faces.TryGetValue("north", out var face3))
        {
            var faceUVs = GetFaceUVs(face3);
            AddFaceSurface(arrayMesh,
                new Vector3(-halfSize.X, -halfSize.Y, -halfSize.Z),
                new Vector3(-halfSize.X, halfSize.Y, -halfSize.Z),
                new Vector3(halfSize.X, halfSize.Y, -halfSize.Z),
        ...
        // South face (z+)
        if (faces != null && faces.TryGetValue("south", out var face4))
        {
            var faceUVs = GetFaceUVs(face4);
            AddFaceSurface(arrayMesh,
                new Vector3(halfSize.X, -halfSize.Y, halfSize.Z),
                new Vector3(halfSize.X, halfSize.Y, halfSize.Z),
                new Vector3(-halfSize.X, halfSize.Y, halfSize.Z),
        ...
        // West face (x-)
        if (faces != null && faces.TryGetValue("west", out var face5))
        {
            var faceUVs = GetFaceUVs(face5);
            AddFaceSurface(arrayMesh,
                new Vector3(-halfSize.X, -halfSize.Y, halfSize.Z),
                new Vector3(-halfSize.X, halfSize.Y, halfSize.Z),
                new Vector3(-halfSize.X, halfSize.Y, -halfSize.Z),
        ...
        // East face (x+)
        if (faces != null && faces.TryGetValue("east", out var face6))
        {
            var faceUVs = GetFaceUVs(face6);
            AddFaceSurface(arrayMesh,
                new Vector3(halfSize.X, -halfSize.Y, -halfSize.Z),
                new Vector3(halfSize.X, halfSize.Y, -halfSize.Z),
                new Vector3(halfSize.X, halfSize.Y, halfSize.Z),
    ...
    public static Node3D CreateNodeFromModel(MinecraftModel model, float xRotation = 0, float yRotation = 0)
    {
        ...
        foreach (var element in resolvedModel.Elements)
        {
            var mesh = CreateMeshFromElement(element, resolvedModel.Textures, pivotOffset, rotationBasis,
    ...
    /// </summary>
    private static void ApplyTexturesToMesh(MeshInstance3D meshInstance, Dictionary<string, ElementFace> faces,
    ...
    private static ImageTexture LoadMinecraftTexture(string texturePath)
    {
        ...
        var dataPath = Path.Combine(userDataPath, "data");

        // Search in multiple asset folders
        var assetFolders = new List<string>();
        if (Directory.Exists(dataPath))
        {
            assetFolders.AddRange(Directory.GetDirectories(dataPath)
                .Where(f => Path.GetFileName(f).Contains("Assets", StringComparison.OrdinalIgnoreCase)));
        }
        ...
        foreach (var assetFolder in assetFolders)
        {
            ...
            {
                Path.Combine(assetFolder, "assets", namespaceName, "textures", actualPath + ".png"),
                Path.Combine(assetFolder, "assets", namespaceName, "textures", actualPath.Replace("/", "\\") + ".png"),
            };
    ...
    private static MinecraftModel ResolveModelParents(MinecraftModel model)
    {
        ...
        foreach (var m in chain)
        {
            ...
            if (m.Elements is { Count: > 0 } && resolved.Elements.Count == 0)
            {
        ...
        do
        {
            ...
            foreach (var key in model.Textures.Keys.ToList())
            {
                ...
                if (value.StartsWith('#'))
                {
                    ...
                    if (model.Textures.TryGetValue(varName, out string value1))
                    {
                        model.Textures[key] = value1;
                        resolved = true;
                    }
                }
            }
    ...
    private static MinecraftModel GetModelByPath(string path)
    {
        ...
        foreach (var pattern in patterns.Where(pattern => loader.HasModel(pattern)))
        {
            return loader.GetModel(pattern);
        }
        ...
        var segmentMatches = allPaths.Where(p =>
        {
            ...
            var pLast = pSegments[^1].Replace(".json", "");
    ...
    public static List<string> GetBlockStateVariants(string blockStateName)
    {
        ...
        return blockState.Variants != null ? blockState.Variants.Keys.ToList() : [];
    }
    ...
    private static Node3D CreateNodeFromMultipartBlockState(BlockState blockState)
    {
        ...
        foreach (var part in blockState.Multipart)
        {
            ...
            try
            {
                if (part.Apply is JsonElement jsonElement)
                {
        ...
        if (rootNode.GetChildCount() != 0) return rootNode;
    ...
    {
        if (blockState.Variants == null || !blockState.Variants.TryGetValue(variant, out object variantObj))
            return null;
        ...
        try
        {
            ...
            if (variantObj is JsonElement jsonElement)
            {
                if (jsonElement.ValueKind == JsonValueKind.Array)
                {

...
public partial class ProjectPropertiesPanel : Panel
{
    ...
    public Node3D FloorNode { get; private set; }

    public MeshInstance3D BackgroundColorNode { get; private set; }

    public MeshInstance3D BackgroundImageNode { get; private set; }
    ...
    public float TextureAnimationFps =>
        _textureAnimationFpsSpinBox != null ? (float)_textureAnimationFpsSpinBox.Value : 20f;
    ...
    private void OnProjectClosed()
    {
        ...
        if (BackgroundColorNode?.MaterialOverride is ShaderMaterial colorShaderMat)
        {
    ...
    private void SetupUi()
    {
        ...
        var frameratePresets = new[] { 24, 30, 60, 120 };
        ...
        var textureAnimationPresets = new[] { 10, 20, 30, 60 };
        ...
        _useAdvancedSkyCheckbox = new CheckBox
        {
            ...
            ButtonPressed = false,
            TooltipText =
                "Replaces the Minecraft sky with a volumetric advanced sky (SunshineClouds2). Hides Minecraft clouds."
        };
        ...
        foreach (var (name, capturedColor) in presets)
        {
            var presetButton = new Button
            {
        ...
        HookSpinBoxUndo(_sunRotationXSpinBox,
            ...
            v =>
            {
                _sunRotationXSpinBox.SetValueNoSignal(v);
                ApplySunRotation();
            });
        ...
        HookSpinBoxUndo(_sunRotationYSpinBox,
            ...
            v =>
            {
                _sunRotationYSpinBox.SetValueNoSignal(v);
                ApplySunRotation();
            });
        ...
        HookSpinBoxUndo(_sunRotationZSpinBox,
            ...
            v =>
            {
                _sunRotationZSpinBox.SetValueNoSignal(v);
                ApplySunRotation();
            });
    ...
    private void FindBackgroundNodes()
    {
        ...
        BackgroundColorNode = subViewport.GetNodeOrNull<MeshInstance3D>("BackgroundColor");
        BackgroundImageNode = subViewport.GetNodeOrNull<MeshInstance3D>("BackgroundImage");
    ...
    {
        if (BackgroundImageNode?.MaterialOverride is ShaderMaterial shaderMat)
        {
            var tex = shaderMat.GetShaderParameter("albedo_tex").As<Texture2D>();
            if (tex is CompressedTexture2D compressedTex)
            {
    ...
    private string GetAbsolutePathForLoading(string path)
    {
        ...
        var absolutePath = Path.Combine(projectFolder, path);
            ...
            EditorCommandHistory.Instance.PushWithoutExecute(
                new PropertyChangeCommand<bool>(
                    ...
                    v =>
                    {
                        ...
                        if (BackgroundImageNode?.MaterialOverride is ShaderMaterial sm)
    ...
    private void ApplySkySetting(bool useSky)
    {
        ...
        if (useSky)
        {
            ...
            if (BackgroundColorNode != null)
                BackgroundColorNode.Visible = false;
        }
        else
        {
            ...
            if (BackgroundColorNode != null)
                BackgroundColorNode.Visible = true;
        }
    ...
    private void FindFloorNode()
    {
        ...
        FloorNode = subViewport.GetNode<Node3D>("Floor");
        if (FloorNode == null)
        {
        ...
        if (FloorNode.GetChildCount() > 0)
        {
            _floorMeshInstance = FloorNode.GetChild<MeshInstance3D>(0);
    ...
    public AdvancedSkySettings GetAdvancedSkySettings()
    {
        return new AdvancedSkySettings
        {
            ...
            SunDiscFeather = _advSkySunDiscFeatherSpinBox != null
                ? (float)_advSkySunDiscFeatherSpinBox.Value
                : float.NaN,
            SunDiscIntensity = _advSkySunDiscIntensitySpinBox != null
                ? (float)_advSkySunDiscIntensitySpinBox.Value
                : float.NaN,
    ...
    private void LoadCurrentSkySettings()
    {
        ...
        if (hasSavedSkyColors)
        {
            ...
            TrySetColorPicker(_skyHorizonDayColorPicker, settings.SkyHorizonDayColor,
                new Color(0.576f, 0.608f, 1.0f, 1.0f));
            TrySetColorPicker(_skyZenithDayColorPicker, settings.SkyZenithDayColor,
                new Color(0.12f, 0.25f, 0.55f, 1.0f));
            TrySetColorPicker(_skyHorizonSunsetColorPicker, settings.SkyHorizonSunsetColor,
                new Color(1.0f, 0.45f, 0.2f, 1.0f));
            TrySetColorPicker(_skyZenithSunsetColorPicker, settings.SkyZenithSunsetColor,
                new Color(0.3f, 0.1f, 0.35f, 1.0f));
            TrySetColorPicker(_skyNightHorizonColorPicker, settings.SkyNightHorizonColor,
                new Color(0.05f, 0.05f, 0.15f, 1.0f));
            TrySetColorPicker(_skyNightZenithColorPicker, settings.SkyNightZenithColor,
                new Color(0.01f, 0.01f, 0.03f, 1.0f));
    ...
    private void ResetSkySettings()
    {
        ...
        if (_skyHorizonSunsetColorPicker != null)
            _skyHorizonSunsetColorPicker.Color = new Color(1.0f, 0.45f, 0.2f, 1.0f);
        if (_skyZenithSunsetColorPicker != null) _skyZenithSunsetColorPicker.Color = new Color(0.3f, 0.1f, 0.35f, 1.0f);
        if (_skyNightHorizonColorPicker != null)
            _skyNightHorizonColorPicker.Color = new Color(0.05f, 0.05f, 0.15f, 1.0f);
    ...
    private static void TrySetColorPicker(ColorPickerButton picker, string htmlColor, Color defaultColor)
    {
        ...
        if (!string.IsNullOrEmpty(htmlColor))
        {
            try
            {
                picker.Color = new Color(htmlColor);
                return;
            }
            catch
            {
    ...
    private void LoadCurrentFloorSettings()
    {
        ...
        if (FloorNode != null)
        {
            ...
            bool floorVisible = hasSavedFloorSettings ? settings.FloorVisible : FloorNode.Visible;
            _floorVisibilityCheckbox.SetPressedNoSignal(floorVisible);
            FloorNode.Visible = floorVisible;
        }
    ...
    {
        var old = FloorNode?.Visible ?? !visible;
        if (FloorNode != null)
        {
            FloorNode.Visible = visible;
        }

        if (old != visible && EditorCommandHistory.Instance != null)
        {
            var capturedOld = old;
            var capturedNew = visible;
            EditorCommandHistory.Instance.PushWithoutExecute(
                new PropertyChangeCommand<bool>(
                    ...
                    v =>
                    {
                        ...
                        if (FloorNode != null) FloorNode.Visible = v;
                    }));
        }
    }
    ...
    private void OnFloorTextureSelected(long index)
    {
        ...
        {
            var capturedOld = oldIndex;
            var capturedNew = index;
            EditorCommandHistory.Instance.PushWithoutExecute(
    ...
    private void OnResolutionPresetPressed(int width, int height)
    {
        ...
        {
            var capturedOldW = oldW;
            var capturedOldH = oldH;
            var capturedNewW = (double)width;
            var capturedNewH = (double)height;
            EditorCommandHistory.Instance.PushWithoutExecute(
    ...
    private void OnFrameratePresetPressed(int fps)
    {
        ...
        {
            var capturedOld = oldFps;
            var capturedNew = (double)fps;
            EditorCommandHistory.Instance.PushWithoutExecute(

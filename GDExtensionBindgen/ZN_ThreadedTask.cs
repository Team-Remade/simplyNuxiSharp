...
public class ZN_ThreadedTask
{
	// Engine object used for calling engine methods
	protected RefCounted _object;

	public ZN_ThreadedTask() : this(NativeName)
	{
	}
	protected ZN_ThreadedTask(StringName @class) : this(ClassDB.Instantiate(@class))
	{
	}
	protected ZN_ThreadedTask(Variant variant) : this((RefCounted) variant)
	{
	}
	protected ZN_ThreadedTask([NotNull] RefCounted @object)
	{
		_object = @object;
	}

	public static implicit operator RefCounted(ZN_ThreadedTask self) => self?._object;
	public static implicit operator Variant(ZN_ThreadedTask self) => self?._object;
	public static explicit operator ZN_ThreadedTask(Variant variant) => variant.AsGodotObject() != null ? new(variant) : null;
	...
	private static readonly StringName NativeName = "ZN_ThreadedTask";
	...
	public event Action Completed
	{
		add
		{
			Connect(SignalName.Completed, Callable.From(value));
		}
		remove
		{
			Disconnect(SignalName.Completed, Callable.From(value));
		}
	}

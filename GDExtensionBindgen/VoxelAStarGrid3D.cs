...
public class VoxelAStarGrid3D
{
	// Engine object used for calling engine methods
	protected RefCounted _object;

	public VoxelAStarGrid3D() : this(NativeName)
	{
	}
	protected VoxelAStarGrid3D(StringName @class) : this(ClassDB.Instantiate(@class))
	{
	}
	protected VoxelAStarGrid3D(Variant variant) : this((RefCounted) variant)
	{
	}
	protected VoxelAStarGrid3D([NotNull] RefCounted @object)
	{
		_object = @object;
	}

	public static implicit operator RefCounted(VoxelAStarGrid3D self) => self?._object;
	public static implicit operator Variant(VoxelAStarGrid3D self) => self?._object;
	public static explicit operator VoxelAStarGrid3D(Variant variant) => variant.AsGodotObject() != null ? new(variant) : null;
	...
	public class MethodName : RefCounted.MethodName
	{
		...
		public static readonly StringName FindPathAsync = "find_path_async";
		public static readonly StringName IsRunningAsync = "is_running_async";
		public static readonly StringName DebugGetVisitedPositions = "debug_get_visited_positions";
	}

	public class SignalName : RefCounted.SignalName
	{
		public static readonly StringName AsyncSearchCompleted = "async_search_completed";
	}

	private static readonly StringName NativeName = "VoxelAStarGrid3D";

	#region Methods

	public void SetTerrain(VoxelTerrain @terrain)
	{
		_object.Call(MethodName.SetTerrain, @terrain);
	}

	public void SetRegion(Godot.Aabb @box)
	{
		_object.Call(MethodName.SetRegion, @box);
	}

	public Godot.Aabb GetRegion()
	{
		return (Godot.Aabb)_object.Call(MethodName.GetRegion);
	}

	public Godot.Collections.Array<Godot.Vector3I> FindPath(Godot.Vector3I @from_position, Godot.Vector3I @to_position)
	{
		return (Godot.Collections.Array<Godot.Vector3I>)_object.Call(MethodName.FindPath, @from_position, @to_position);
	}

	public void FindPathAsync(Godot.Vector3I @from_position, Godot.Vector3I @to_position)
	{
		_object.Call(MethodName.FindPathAsync, @from_position, @to_position);
	}

	public bool IsRunningAsync()
	{
		return (bool)_object.Call(MethodName.IsRunningAsync);
	}

	public Godot.Collections.Array<Godot.Vector3I> DebugGetVisitedPositions()
	{
		return (Godot.Collections.Array<Godot.Vector3I>)_object.Call(MethodName.DebugGetVisitedPositions);
	}
	...
	public event Action<Godot.Collections.Array<Godot.Vector3I>> AsyncSearchCompleted
	{
		add
		{
			Connect(SignalName.AsyncSearchCompleted, Callable.From(value));
		}
		remove
		{
			Disconnect(SignalName.AsyncSearchCompleted, Callable.From(value));
		}
	}

...
public class VoxelVoxLoader
{
	// Engine object used for calling engine methods
	protected RefCounted _object;

	public VoxelVoxLoader() : this(NativeName)
	{
	}
	protected VoxelVoxLoader(StringName @class) : this(ClassDB.Instantiate(@class))
	{
	}
	protected VoxelVoxLoader(Variant variant) : this((RefCounted) variant)
	{
	}
	protected VoxelVoxLoader([NotNull] RefCounted @object)
	{
		_object = @object;
	}

	public static implicit operator RefCounted(VoxelVoxLoader self) => self?._object;
	public static implicit operator Variant(VoxelVoxLoader self) => self?._object;
	public static explicit operator VoxelVoxLoader(Variant variant) => variant.AsGodotObject() != null ? new(variant) : null;
	...
	private static readonly StringName NativeName = "VoxelVoxLoader";

	#region Methods

	public static int LoadFromFile(string @fpath, VoxelBuffer @voxels, VoxelColorPalette @palette, VoxelBuffer.ChannelId @dst_channel = (VoxelBuffer.ChannelId)2)
	{
		return (int)ClassDB.ClassCallStatic(NativeName, MethodName.LoadFromFile, @fpath, @voxels, @palette, (int)@dst_channel);
	}

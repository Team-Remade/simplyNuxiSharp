...
public partial class Main : Control
{
    ...
    /// </summary>
    private async Task ShowProjectScreenAndWait()
    {
    ...
    /// </summary>
    public Task SpawnModelFromPathAsync(string modelPath)
    {
    ...
    private void ShowOpenProjectDialog()
    {
        NativeFileDialog.ShowOpenFile(
            "Open Project",
            ["*.srproject ; Simply Remade Project"],
            (success, filePath) =>
            {
    ...
    public async Task RestoreSceneWithProgressAsync()
    {
    ...
    private void OnSaveProject()
    {
        if (string.IsNullOrEmpty(ProjectManager.CurrentProjectFile))
        {
            ...
            dlg.DialogText =
                "Please create or open a project before saving.\n\nUse File → New Project or File → Open Project.";
            dlg.OkButtonText = "OK";
            dlg.Exclusive = true;
            dlg.Transient = true;
            dlg.CloseRequested += () =>
    ...
    private void ShowRenderAnimationDialog()
    {
        ...
        if (lastKeyframe == 0)
        {
            ...
            errorDialog.DialogText =
                "Cannot render animation: No keyframes found in the timeline.\n\nPlease add keyframes to your animation before rendering.";
        ...
        {
            PreviewViewportControl.RenderAnimation(outputPath, format, isPngSequence, bitrateMbps, width, height,
                framerate, lastKeyframe);
        });
    ...
    private void ShowUnsavedChangesDialog(Action onQuit)
    {
        ...
        dlg.DialogText =
            $"The project \"{ProjectManager.CurrentProjectName}\" has unsaved changes.\n\nDo you want to save before closing?";
    ...
    private async Task ToggleRenderMode()
    {
        ...
        if (PreviewViewportControl != null) await ToggleTAA(PreviewViewportControl.PreviewSubViewport);
    }
    ...
    private void UpdateLightsRenderMode(Node node, bool enabled)
    {
        foreach (var child in node.GetChildren())
        {
            ...
            {
                UpdateLightsRenderMode(child, enabled);
            }
        }
